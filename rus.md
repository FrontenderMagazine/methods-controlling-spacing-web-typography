...
Здесь нам поможет свойство [`word-spacing`][14], которое, к тому же, отлично поддерживается браузерами. Вот отрывок текста, набранный шрифтом [Prompt][15], который несколько шире большинства и в данном случае текст будет выглядеть лучше, если немного подкоректировать расстояния между словами.
...
Не все шрифты одинаково отображаются в различных операционных системах. Это происходит потому, что в каждой операционной системе, будь то Windows, MacOS или любая другая, разные процессы управляют тем, сколько пикселей используется для отображения шрифтов.
